...
DROP FUNCTION IF EXISTS al.update_house_lead;
...
BEGIN
  WITH ins AS (
    INSERT INTO al.d_house_lead
      (created_at, updated_at, enc_entity_id, pk_val, pk_field, aid, info)
      SELECT created_at,
             now(),
             enc_entity_id,
             -- mandatory values in payload
             trim(coalesce(payload2 ->> 'pk_val', 'nil')),
             coalesce(payload2 ->> 'pk_field', 'email'),
             aid,
             payload2
      FROM (
             ...
             SELECT max(created_at) as created_at, aid, enc_entity_id, al.jsonb_object_merge2(payload) as payload2
             FROM al.activity
             WHERE event = 'user_assign'
               AND updated_at >= low_wm
               AND updated_at < high_wm
             GROUP BY aid, enc_entity_id, trim(coalesce(payload ->> 'pk_val', 'nil'))) sq1
