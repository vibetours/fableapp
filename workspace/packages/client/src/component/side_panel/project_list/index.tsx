      ...
      <Tags.ProjectsCon showProjects={showAllProjects}>
        {
          Projects.map((project) => {
            return (
              <Tags.ProjectCon>
                <Tags.ProjectBtn onClick={() => {
                  if (showProject.includes(project.id)) {
                    setShowProject(showProject.filter((id) => id !== project.id))
                  } else {
                    setShowProject([...showProject, project.id])
                  }
                }}>
                  <div>
                    <Tags.RightArrow showProjects={showProject.includes(project.id)} src={iconArrowRight} alt={'illustration right arrow'} />
                    <p>{project.name}</p>
                  </div>
                </Tags.ProjectBtn>
                <Tags.ProjectItems showProjects={showProject.includes(project.id)}>
                  {
                    project.projects.map((pr) => {
                      return (
                        <Tags.ProjectItem>
                          <Tags.ProjectItemBtn onClick={() => {
                            if (showProject.includes(pr.id)) {
                              setShowProject(showProject.filter((id) => id !== pr.id))
                            } else {
                              setShowProject([...showProject, pr.id])
                            }
                          }}>
                            <div>
                              <Tags.RightArrow showProjects={showProject.includes(pr.id)} src={iconArrowRight} alt={'illustration right arrow'} />
                              <p>{pr.name}</p>
                            </div>
                          </Tags.ProjectItemBtn>
                          <Tags.ProjectItemList showProjects={showProject.includes(pr.id)}>
                            {
                              pr.items.map((item) => {
                                return (
                                  <li>
